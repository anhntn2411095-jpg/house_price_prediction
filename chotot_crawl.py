...
class Listing:
    ...
    price_per_m2: Optional[float]  # Use for cleaning only, NOT model input
...
def get_first(ad: dict, keys: list[str]) -> Any:
    ...
    This is useful because API field names may vary.
    """
    for key in keys:
        value = ad.get(key)
...
def build_source_url(list_id: int) -> str:
    ...
    Exact frontend URL format may vary, but this gives a useful source identifier.
    """
...
def normalize_legal_document(value: Any) -> Optional[str]:
    """
    Convert legal_document to string category.
    """
    if value is None or value == "":
        return None
